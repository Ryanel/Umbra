...
virt_addr_t* kernel::virt_mm::mmap(virt_addr_t virt, size_t length, int protection, int flags) {
    ...
    for (size_t i = 0; i < length; i += 0x1000) {
        if ((flags & VMM_FLAG_POPULATE) != 0) {
            ...
            kernel::log::trace("vmm", "mmap: 0x%08x (on demand)\n", aligned_vaddr + i);
...
virt_addr_t* kernel::virt_mm::mmap_direct(virt_addr_t virt, phys_addr_t phys, int protection, int flags) {
    kernel::critical_section cs;
    virt_addr_t              aligned_vaddr = get_virt_addr(virt & 0xFFFFF000, flags);
...
virt_addr_t kernel::virt_mm::get_virt_addr(virt_addr_t addr, int flags) {
    if (addr == 0) {
