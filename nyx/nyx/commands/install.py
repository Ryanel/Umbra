    ...
    def run(self):
        if (len(self.args.packages) > 0):
            ...
            for x in reversed_list:
                ...
                elif x.state["installed"] and self.args.rebuild_deps and not x.isTool:
                    table.add_row(x.name,
                        x.version,
                        "Rebuild Dependency"
                    )
            ...
            for x in deps_in_install_order:
                if not x.state["installed"] or x.name in self.args.packages and self.args.rebuild or x.state["installed"] and self.args.rebuild_deps and not x.isTool:
                    rb = x.name in self.args.packages and self.args.rebuild or x.state["installed"] and self.args.rebuild_deps and not x.isTool
